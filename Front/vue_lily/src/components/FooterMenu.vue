...
async function subscribePostRequest() {
  try {
    ...
    await axios.post('http://127.0.0.1:8000/api/v1/products/', { email: email_user });
...
<template>
  <div class="footer">
    <div class="left-column">
      ...
      <p>Copyright © 2023 La Fluer Lily. Powered by La Fluer Lily.</p>
    </div>

    <div class="right-column">
      <h2>Subscribe</h2>
      <form method="post">
        <input  type="email" id="subscration" name="email" placeholder="Your email adress..." />
        <button @click.prevent="subscribePostRequest()" type="submit" class="subs-btn" name="action" value="subscribe">Subscribe</button>
      </form>
    </div>
  </div>
</template>
